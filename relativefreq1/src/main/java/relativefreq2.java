...
public class relativefreq2 {
    ...
    public static class WordPairPartitioner extends Partitioner<wordpair2, IntWritable> {

        @Override
        public int getPartition(wordpair2 wordpair2, IntWritable intWritable, int numPartitions) {
            int check = wordpair2.getWord().hashCode() % numPartitions;
    ...
    public static class PairsRelativeOccurrenceMapper extends Mapper<LongWritable, Text, wordpair2, IntWritable> {
        private wordpair2 wordpair2 = new wordpair2();
        ...
        protected void map(LongWritable key, Text value, Mapper.Context context) throws IOException, InterruptedException {
            ...
            if (tokens.length > 1) {
                for (int i = 0; i < tokens.length; i++) {
                    ...
                    wordpair2.setWord(tokens[i]);

                    int start = (i - neighbors < 0) ? 0 : i - neighbors;
                    int end = (i + neighbors >= tokens.length) ? tokens.length - 1 : i + neighbors;
                    for (int j = start; j <= end; j++) {
                        if (j == i) continue;
                        wordpair2.setNeighbor(tokens[j].replaceAll("\\W", ""));
                        context.write(wordpair2, ONE);
                    }
                    wordpair2.setNeighbor("*");
                    totalCount.set(end - start);
                    context.write(wordpair2, totalCount);
                }
            }
        }
    }
